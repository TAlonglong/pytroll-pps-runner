...
def ready2run(msg, files4pps):
    ...
    if platform_name in SUPPORTED_METOP_SATELLITES:
        if 'antenna' in msg.data:
            ...
            if (msg.data['antenna'] == "ears" or
                msg.data['antenna'] == "global" or
                msg.data['antenna'] == "global-segments"):
                LOG.debug("is ears or global")
                number_of_sensors = 1
        elif 'env' in msg.data:
            LOG.debug("env in msg.data {}".format(msg.data['env']))
            if (msg.data['env'] == "ears" or
                msg.data['env'] == "global" or
                msg.data['env'] == "global-segments"):
                LOG.debug("is ears or global")
                number_of_sensors = 1
